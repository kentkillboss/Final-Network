...
const Mole = ({ active = false, loading = false, onWhack, speed, delay, points, pointsMin = 10 }) => {
  ...
  const setMole = useCallback((override, accent = 45, shades = 1, golden = 1, hue = 45, lightness = 65) => {
    ...
    if (Math.random() > 0.99 || override) {
      ...
      gsap.set(moleContainerRef.current, {
        ...
        '--golden': 0,
        '--hue': Math.random() > 0.5 ? gsap.utils.random(185, 215) : gsap.utils.random(30, 50),
  ...
  useEffect(() => {
    ...
    if (active) {
      ...
      bobRef.current = gsap.to([buttonRef.current, moleRef.current], {
        ...
        repeatDelay: delay,
        onRepeat: () => {
          pointsRef.current = Math.floor(Math.max(pointsRef.current * constants.POINTS_MULTIPLIER, pointsMin));
        },
      });
    }
      ...
      gsap.to([moleRef.current, buttonRef.current], {
        ...
        onComplete: () => {
          delayedRef.current = gsap.delayedCall(gsap.utils.random(1, 3), () => {
            setMole();
            setWhacked(false);
            bobRef.current.restart().timeScale(bobRef.current.timeScale() * constants.TIME_MULTIPLIER);
          });
        },
      });
    }
      ...
      >
        ...
        <g clipPath="url(#mole-clip)">
          <g className="mole__mole" ref={moleRef}>
            ...
            />
            <rect ref={tummyRef} x="45.5" y="155" width="110" height="123" rx="55" className="mole__white" />
            ...
            <g ref={faceRef}>
              <g clipPath="url(#muzzle-clip)">
                <ellipse className="mole__shadow" cx="100.5" cy="107" rx="40" ry="25" />
                <ellipse className="mole__white" cx="100.5" cy="103" rx="40" ry="25" />
              </g>
              <path
              ...
              />
              <ellipse ref={noseRef} className="mole__nose" cx="100.5" cy="91" rx="10" ry="6" />
            </g>
            <g className="specs" ref={specsRef}>
              <circle cx="53" cy="84" r="12" className="specs__lens" strokeWidth="4" />
              <circle className="specs__lens" cx="148" cy="84" r="12" strokeWidth="4" />
              <path className="specs__bridge" d="M65 84s14-6 36.5-6 34.5 6 34.5 6" stroke="#000" strokeWidth="4" />
            ...
            <g className="mole__cap" ref={capRef}>
              ...
              <clipPath id="cap-clip" maskUnits="userSpaceOnUse" x="22" y="8" width="157" height="57">
                <path d="M99.5 8C71 8 29 25.5 22 64.5h157C173.5 21.5 128 8 99.5 8z" />
              </clipPath>
              <g clipPath="url(#cap-clip)">
                <path ref={capBody} d="M-10 8h220v57h-89.5V51.5H82V65h-92V8z" className="cap__body" />
              </g>
              <ellipse cx="100.5" cy="8.5" rx="6" ry="2.5" className="cap__accent" />
            </g>
          </g>
        </g>
        <g clipPath="url(#mole-clip)">
...
const WhacAMole = () => {
  const { play: playCount } = useAudio('https://assets.codepen.io/605876/countdown-beep.mp3');
  const { play: playWhack } = useAudio('https://assets.codepen.io/605876/pop.mp3');
  const { play: playSqueak } = useAudio('https://assets.codepen.io/605876/squeak-in.mp3');
  const { play: playSqueakOut } = useAudio('https://assets.codepen.io/605876/squeak-out.mp3');
  const { play: playCheer } = useAudio('https://assets.codepen.io/605876/kids-cheering.mp3');
  const { play: playThud } = useAudio('https://assets.codepen.io/605876/thud--small.mp3', 0.65);
  const { play: playWhistle } = useAudio('https://assets.codepen.io/605876/whistle.mp3', 0.65);
  const { play: playSparkle } = useAudio('https://assets.codepen.io/605876/sparkle.mp3');
  const { play: playClick } = useAudio('https://assets.codepen.io/605876/click.mp3');
  ...
  return (
    <div className="body1" style={{ zIndex: '1000000' }}>
      <Fragment className="body1">
        ...
        {/* Fresh */}
        {!starting && !playing && !finished && <StartScreen onStart={startGame} />}
        {/* Starting */}
        {starting && (
          <CountDown
        ...
        {playing && (
          <Fragment>
            <button className="icon-button end-button" onClick={endGame}>
              ...
              <svg className="icon" viewBox="0 0 352 512" width="100" title="times">
        ...
        <div className="main1" ref={boardRef}>
          <div className="moles">
        ...
        {/* Finished */}
        {finished && <FinishScreen onRestart={startGame} onReset={resetGame} newHigh={newHighScore} result={score} />}
      </Fragment>
    </div>
  );
};
